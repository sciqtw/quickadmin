...
class AliossStorage extends Storage
{
    ...
    {
        // 读取配置文件
        $this->point = sysConfig('storage.alioss_point');
        $this->bucket = sysConfig('storage.alioss_bucket');
        $this->accessKey = sysConfig('storage.alioss_access_key');
        $this->secretKey = sysConfig('storage.alioss_secret_key');
        // 计算链接前缀
        $type = strtolower(sysConfig('storage.alioss_http_protocol'));
        $domain = strtolower(sysConfig('storage.alioss_http_domain'));
