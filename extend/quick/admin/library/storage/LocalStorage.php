...
class LocalStorage extends Storage
{
    ...
    {
        $type = sysConfig('storage.local_http_protocol') ?: 'follow';
        if ($type === 'follow') $type = $this->app->request->scheme();
        $this->prefix = trim(dirname($this->app->request->baseFile(false)), '\\/');
        if ($type !== 'path') {
            $domain = sysConfig('storage.local_http_domain') ?: $this->app->request->host();
    ...
    public function info(string $name, bool $safe = false, ?string $attname = null): array
    {
        return $this->has($name, $safe) ? [
            ...
            'key' => "upload/{$name}", 'file' => $this->savePath($name, $safe),
        ] : [];
    }
