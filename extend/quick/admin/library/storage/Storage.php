...
abstract class Storage
{
    ...
    {
        $this->app = $app;
        $this->link = 'none';//sysconf('storage.link_type');
        $this->initialize();
    }
    ...
    {
        $class = ucfirst(strtolower($name ?: sysConfig('storage.type')));
        if (class_exists($object = "quick\\admin\\library\\storage\\{$class}Storage")) {
    ...
    public static function down(string $url, bool $force = false, int $expire = 0): array
    {
        try {
            ...
            if (empty($force) && $file->has($name)) {
                if ($expire < 1 || filemtime($file->savePath($name)) + $expire > time()) {
                    return $file->info($name);
                }
            }
            return $file->save($name, static::curlGet($url));
