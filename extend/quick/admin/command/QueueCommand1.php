...
class QueueCommand extends Command
{
    ...
    protected function cleanAction()
    {
        ...
        $clear = SystemQueue::make()->where($map)->delete();
        // 标记超过 1 小时未完成的任务为失败状态，循环任务失败重置
        $map1 = [['loops_time', '>', 0], ['status', '=', 4]]; // 执行失败的循环任务
        $map2 = [['exec_time', '<', time() - 3600], ['status', '=', 2]]; // 执行超时的任务
        [$timeout, $loops, $total] = [0, 0, SystemQueue::make()->whereOr([$map1, $map2])->count()];
        SystemQueue::make()->whereOr([$map1, $map2])->chunk(100, function (Collection $result) use ($total, &$loops, &$timeout) {
            foreach ($result->toArray() as $item) {
                ...
                SystemQueue::make()->where(['id' => $item['id']])->update(['status' => $status, 'exec_desc' => $message]);
            }
        });
    ...
    protected function listenAction()
    {
        ...
        while (true) {
            ...
            foreach (SystemQueue::make()->where($map)->order('exec_time asc')->cursor() as $vo) try {
                $args = "quick:queue dorun {$vo['code']} -";
                ...
                SystemQueue::make()->where(['code' => $vo['code']])->update([
    ...
    protected function doRunAction()
    {
        ...
        if (empty($this->code)) {
            ...
            if (empty($this->queue->record) || intval($this->queue->record['status']) !== 1) {
                ...
                // 锁定任务状态，防止任务再次被执行
                SystemQueue::make()->strict(false)->where(['code' => $this->code])->update([
                ...
                if (class_exists($command = $this->queue->record['command'])) {
                    ...
                    if ($class instanceof QueueCommand) {
                        ...
                        throw new \quick\admin\Exception("自定义 {$command} 未继承 Queue 或 QueueService");
                    }
                } else {
