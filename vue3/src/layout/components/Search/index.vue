        ...
        <div class="container">
            <div class="search-box" @click.stop>
                ...
                <div v-if="$store.state.settings.mode === 'pc'" class="tips">
                    ...
                    <div class="tip">
                        <span><svg-icon icon="search-up" /></span>
                        <span><svg-icon icon="search-down" /></span>
                        切换搜索结果
                    </div>
                    <div class="tip">
                        <span><svg-icon icon="search-enter" /></span>
                        访问页面
                    </div>
                    <div class="tip">
                        <span>ESC</span>
                ...
                <router-link v-for="(item, index) in resultList" :key="item.path" v-slot="{ href, navigate }" custom :to="isShow ? item.path : ''">
                    <a :ref="`search-item-${index}`" :href="isExternal(item.path) ? item.path : href" class="item" :class="{'actived': index === actived}" :target="isExternal(item.path) ? '_blank' : '_self'" @click="navigate" @mouseover="actived = index">
                        ...
                        <div class="info">
                            <div class="title">
                                {{ item.title }}
                                <svg-icon v-if="item.isExternal" icon="external-link" />
                            </div>
                            <div class="breadcrumb">
                                <span v-for="(bc, bcIndex) in item.breadcrumb" :key="bcIndex">
                                    {{ bc }}
                                    <svg-icon icon="el-icon-arrow-right" />
                                </span>
                            </div>
...
<script setup>
  import { getCurrentInstance, computed , watch ,onMounted ,ref} from 'vue';
